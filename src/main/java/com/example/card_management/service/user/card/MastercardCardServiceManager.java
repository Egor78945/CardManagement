...
public class MastercardCardServiceManager extends UserCardService<UserCard>{
    public MastercardCardServiceManager(UserCardRepository userCardRepository, @Qualifier("stringEncoder") Encoder<String> encoder) {
        super(userCardRepository, encoder);
    }

    @Override
    public void save(UserCard userCard) {
        if (!userCardRepository.existsUserCardByNumber(userCard.getNumber()) && userCardRepository.findCountByOwnerEmailAndTypeId(userCard.getOwnerEmail(), UserCardType.MASTERCARD.getId()) < 5) {
    ...
    public List<UserCard> getAllByOwnerEmailAndCardType(String email, int pageNumber, int pageSize) {
        return userCardRepository.findAllByOwner_EmailAndType_Id(email, UserCardType.MASTERCARD.getId(), PageRequest.of(pageNumber, pageSize))
                .stream()
